      ...
      <Container>
        ...
        <Row>
          ...
          >
            <Link
              onClick={selectDaily}
              className={"roster-link " + (dailyActive ? "active-link" : "")}
            >
              Daily
            </Link>
            <Link
              onClick={selectNormal}
              className={"roster-link " + (normalActive ? "active-link" : "")}
            >
              Normal
            </Link>
            <Link
              onClick={selectWeek}
              className={"roster-link " + (weekActive ? "active-link" : "")}
            >
              Weekly
            </Link>
            <Link
              onClick={selectMonth}
              className={"roster-link " + (monthActive ? "active-link" : "")}
            >
              Monthly
            </Link>
          </Col>
          ...
          {displayNormal && (
            <Col className="right" style={{ padding: "0px" }}>
              ...
              <span>
                <label className="roster-date" style={{ marginRight: "5px" }}>
                  End Date
                </label>
                <DatePicker
                  wrapperClassName="datePicker"
        ...
        {storeID.storeId && displayTable ? (
          <div style={{ marginTop: "10px" }}>
            ...
            {adminRosterUtilisationScheduleResult &&
            adminRosterUtilisationScheduleResult.rosterDates &&
            adminRosterUtilisationScheduleResult.rosterDates.length &&
            rosterLoading ? (
              <ScrollArea
          speed={1}
        ...
        >
              <table className="roster-table roaster-table-sticky">
                <thead className="roaster-header-sticky">
                  <tr>
                    ...
                    {adminRosterUtilisationScheduleResult &&
                      adminRosterUtilisationScheduleResult.rosterDates &&
                      adminRosterUtilisationScheduleResult.rosterDates.length &&
                      adminRosterUtilisationScheduleResult.rosterDates.map(
                        (data) => (
                          <th
                          ...
                          >
                            ...
                            <ul>
                              <li className="bold">
                                {data.generalCount} General
                              </li>
                              <li className="bold">
                                {data.onDutyCount} on duty
                              </li>
                              <li className="bold">
                                {data.captainCount} Captain
                              </li>
                            </ul>
                          </th>
                        )
                ...
                <tbody>
                  {adminRosterUtilisationScheduleResult.rosterResponses &&
                    adminRosterUtilisationScheduleResult.rosterResponses
                      .length &&
                    adminRosterUtilisationScheduleResult.rosterResponses.map(
                      (data, index) => {
                        return (
                          <tr>
                            <td
                            ...
                            >
                              ...
                              >
                                ...
                                <div style={{ margin: "auto" }}>
                                  <CircularProgressbar
                                    value={data.utilization.split("%")[0]}
                            ...
                            {data.employeeRosters.map((empData) => (
                              <td
                                ...
                                style={{
                                  backgroundColor: `${
                                    empData.roster && empData.roster.leave
                                      ? "#FF5370"
                                      : empData.roster && empData.roster.holiday
                                      ? "#FF9F40"
                                      : empData.roster && empData.roster.weekOff
                                      ? "#006EBB"
                                      : empData.roster &&
                                        empData.roster.shiftName
                                      ? "#22AF47"
                                      : "#8d8d8d94"
                                  }`,
                                  color: "white",
                              ...
                              >
                                ...
                                {empData.roster === null ? (
                                  ...
                                  <div style={{ textAlign: "center" }}>
                                    {empData.roster.leave
                                      ? "Leave"
                                      : empData.roster.holiday
                                      ? empData.roster.holiday
                                      : empData.roster.weekOff
                                      ? "WeekOff"
                                      : empData.roster.shiftName
                                      ? empData.roster.shiftName
                                      : "----"}
