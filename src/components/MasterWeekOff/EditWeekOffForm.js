import React, {
  ...
  const EditWeekOffForm = (props) => {
    const { weekOffCreate, weekoffData,weekOffById,weekOffEdit,viewWeekOffById } = useContext(WeekOffContext);
    ...
    useEffect(() => {
if(weekOffById!==null&&weekOffById!==undefined&&weekOffById&&Object.keys(weekOffById).length!==0){
    setWeekOffList({weekOffId: weekOffById.weekOffId,
    contractType: weekOffById.contractType,
    numberOfWeekOffs: weekOffById.numberOfWeekOffs})
    setEffectiveDate(new Date(weekOffById.effectiveDate))
}
      }, [weekOffById]);
  console.log("weekOffById",weekOffById,weekOffList);
    ...
    const submitHandler = (e) => {
      ...
      if (validate) {
        const data = {
          weekOffId: weekOffList.weekOffId,
  ...
  export default EditWeekOffForm;
