...
const FinanceClearanceList = () => {
  const { 
        total,loader,
    saveFinanceClearanceData,separationListView,separationList } = useContext(
    SeparationContext
  );
  const { CostCenter, costCenterList } = useContext(AdminContext)
  const [pageCount, setPageCount] = useState(0);
...
const [currentPage, setCurrentPage] = useState(1);
const recordPerPage = 10;
const totalRecords = separationList !== null && separationList !== undefined && total;
const pageRange = 10;
const indexOfLastRecord = currentPage * recordPerPage;
const indexOfFirstRecord = indexOfLastRecord - recordPerPage;
const [currentRecords, setCurrentRecords] = useState([]);
...
const handleCostCenter = (options) => {
  ...
  const renderStatusOptions = (value) => {
    return (
      <div>
        <select name="financeClearanceStatus" value={value.data.financeClearanceStatus} onChange={(e) => statusRender(e,value)}>
          <option value="0"> Due </option>
          <option value="1"> No Due </option>
          <option value="2"> On Hold </option>
        </select>
      ...
      <div className="container-fluid">
        ...
        <div className="ag-theme-alpine" style={{ align:"center",height: 495, width: 1400 }}>
          ...
          <AgGridColumn className="columnColor" editable="false" headerName="S No" pinned="left" valueGetter={`node.rowIndex+1 + ${indexOfFirstRecord}`}></AgGridColumn>
            ...
            <AgGridColumn className="columnColor" editable="false" headerName="Employee Name" field="empName"></AgGridColumn>
            <AgGridColumn className="columnColor" editable="false" headerName="Cost Center Name" field="costCentre"></AgGridColumn>
            ...
            <AgGridColumn className="columnColor" headerName="Finance Amount To Be Recovered" field="financeAmount"></AgGridColumn>
