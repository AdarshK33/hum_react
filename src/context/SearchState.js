...
export const SearchProvider = ({ children }) => {
  ...
  function searchByEmpId(Id) {
    client
      .get("/api/v1/leave_transaction/view?empId=" + Id)
      .then(function (response) {
        ...
        return dispatch({
          type: "FETCH_EMPID_LIST",
          payload: state.empIdSearchList,
        });
      })
  ...
  function searchByEmpIdManager(Id) {
    client
      .get("/api/v1/leave_transaction/view/manager?empId=" + Id)
      .then(function (response) {
        console.log(response);
        ...
        return dispatch({
          type: "FETCH_EMPIDMANAGER_LIST",
          payload: state.empIdManagerSearchList,
        });
      })
      .catch(function (error) {
        console.log(error);
  ...
  function viewSearchSiftList(Id) {
    client
      .get("/api/v1/shift/search?key=" + Id)
      .then(function (response) {
        if (response.data.data === null) {
          toast.error("No Data Found");
        } else {
  ...
  function viewSearchClusterList(Id) {
    client
      .get("/api/v1/cluster/search?key=" + Id)
      .then(function (response) {
        if (response.data.data === null) {
          toast.error("No Data Found");
        } else {
        ...
        return dispatch({
          type: "FETCH_CLUSTER_SEARCH_LIST",
          payload: state.searchClusterList,
        });
      })
      .catch(function (error) {
        console.log(error);
  ...
  function searchGrantLeave(key) {
    client
      .get("/api/v1/grant_leave/view" + "?key=" + key)
      .then(function (response) {
        if (response.data.data === null) {
          toast.error("No Data Found");
        } else {
        ...
        return dispatch({
          ...
          payload: state.searchGrantLeaveView,
        });
      })
      .catch(function (error) {
        console.log(error);
