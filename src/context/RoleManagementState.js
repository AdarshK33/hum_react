...
export const RoleManagementProvider = ({ children }) => {
  ...
  function viewRoleListData() {
    client
      .get("/api/v1/menu/permission/view")
      .then(function (response) {
        //  console.log(response);
        state.RoleListData = response.data.data;

        return dispatch({
          type: "FETCH_ROLEDATA_LIST",
          payload: state.RoleListData,
        });
      })
      .catch(function (error) {
        console.log(error);
  ...
  function AddRole(values) {
    client
      .post("/api/v1/menu/permission/create", values)
      .then(function (response) {
        //  console.log(response);
        ...
        viewRoleListData();
        return dispatch({
          type: "FETCH_ADDDATA_LIST",
          payload: state.AddNewRole,
        });
      })
      .catch(function (error) {
        console.log(error);
  ...
  function GetRoleData(role) {
    client
      .get("/api/v1/menu/permission/view/" + role)
      .then(function (response) {
        //  console.log(response);
        ...
        for (let j = 0; j < response.data.data.length; j++) {
          for (let i = 0; i < state.MenuList.length; i++) {
            if (response.data.data[j].menuId === state.MenuList[i].menuId) {
              state.EditMenuList.push({
                label: state.MenuList[i].menuName,
                value: state.MenuList[i].menuId,
              });
            }
          }
        }
        ...
        return dispatch({
          type: "FETCH_GETROLEDATA_LIST",
          payload: state.GetRolePermission,
  ...
  function EditRole(values) {
    client
      .post("/api/v1/menu/permission/update", values)
      .then(function (response) {
        //  console.log(response);
        toast.info(response.data.message);
        ...
        viewRoleListData();
        return dispatch({
          type: "FETCH_EDITROLEDATA_LIST",
          payload: state.EditRolePermission,
        });
      })
      .catch(function (error) {
        console.log(error);
