...
export const MasterFilesProvider = ({ children }) => {
  ...
  const viewCountries = () => {
    return client
      ...
      .then((response) => {
        ...
        return dispatch({ type: "VIEW_COUNTRIES", payload: state.countryList });
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  const viewStates = () => {
    return client
      ...
      .then((response) => {
        ...
        return dispatch({ type: "VIEW_STATES", payload: state.stateList });
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  const viewDailyQty = (id, date) => {
    ...
    setLoader(true);
    return client
      .get("/api/v1/daily/view?" + "&storeId=" + id + "&date=" + date)
      .then((response) => {
        console.log(response.data.data);
        state.dailyQty = response.data.data;
        setLoader(false);
        return dispatch({ type: "VIEW_DAILY_QTY", payload: state.dailyQty });
      })
  ...
  const uploadStateFile = (file) => {
    ...
    return client
      .post("/api/v1/state/upload", formData)
      .then((response) => {
  ...
  const uploadDailyQty = (file) => {
    ...
    return client
      .post("/api/v1/daily/upload", formData)
      .then((response) => {
        console.log(response, "res");
        toast.info(response.data.message);
      })
  ...
  const uploadMasterLocation = (file) => {
    ...
    return client
      .post("/api/v1/location/upload", formData)
      .then((response) => {
        console.log(response, "res");
        toast.info(response.data.message);
      })
